...
def lambda_handler(event, context):
    ...
    plan_inputs_table = dynamodb.Table('plan_inputs_dev')
    plan_outputs_table = dynamodb.Table('plan_outputs_dev')
    ...
    # res = dynamodb.Table('default_inputs_dev').put_item(
    #     Item=put_data
    # )
    #
    #
    ...
    if res['ResponseMetadata']['HTTPStatusCode'] == 200:
        ...
        try:
            ...
            if status == 200:
                ...
                # --- NOTE the following code will need to be replace once the time information is created by the optimizer
                # 
                workerData = []
                caseTypeData = []
                # --- iterate through the plan_details output and create the worker data
                rowcount = 0
                for row in res['plan_details']:
                    # --- skip the first and last rows (cases and totals) for workers
                    if (rowcount > 0 and rowcount < (len(res['plan_details']) - 1) ):
                        workerData.append({
                            'name': row[0],
                            'data': row[-1],
                            'location': 'Region ' + str(rowcount + 1)
                        })
                    if (rowcount == 0):
                        colcount = 0
                        for col in row:
                            # --- create the case type
                            if colcount > 0 and colcount < (len(row) - 1):
                                caseTypeData.append({
                                    'name': col,
                                    'data': res['plan_details'][-1][colcount],
                                    'location': 'Region ' + str(rowcount + 1)
                                })
                            colcount += 1
                        #
