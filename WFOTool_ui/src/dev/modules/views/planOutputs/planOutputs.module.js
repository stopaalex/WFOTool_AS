...
var planOutputs = angular.module('planOutputs', [])
    .directive('planOutputs', function () {
        return {
    ...
    .controller('planOutputsCtrl', function ($scope, $rootScope, $route, $location, restService, loaderService, alertService) {
        ...
        // --- currently hard coded - needs to be dynamically updated
        ...
        function updateChart() {
            $scope.chartData.series = $scope.chartDataSeries.map(i => {
        ...
        function generateChart() {
            ...
            // 
            let currentData = JSON.parse(JSON.stringify($scope.outputData.graphicalDisplayData[$scope.chartDisplayType]));

            currentData.forEach((val, idx, arr) => {
                let split = (val.data / $scope.chartDisplayTimeframe).toFixed(2);
                let newDataArr = [];
                for (let i = 0; i < $scope.chartDisplayTimeframe; i++) {
                ...
                if (parseFloat($scope.chartDisplayTimeframe) < 12) {
                    $scope.chartTitle = 'Quartly Distribution';
                    // $scope.chartData.title.text = 'Quartly Distribution';
                    $scope.chartData.xAxis.data = $scope.quarterlyDistribution;
                } else {
                    $scope.chartTitle = 'Monthly Distribution';
                    // $scope.chartData.title.text = 'Monthly Distribution';
        ...
        function downloadPlan(data) {
            ...
            let tab1 = [[], []];
            // let tab1 = d.summary_metrics;
            for (let k in d.summary_metrics) {
            ...
            let tab2 = d.plan_details;


            let csvContent1 = "data:text/csv;charset=utf-8," + tab1.map(e => e.join(",")).join("\n");
            let csvContent2 = "data:text/csv;charset=utf-8," + tab2.map(e => e.join(",")).join("\n");
        ...
        function savePlan(outputData) {
            ...
            $scope.savePlanData = { 'plan_outputs_id': outputData.id, 'plan_inputs_id': outputData.plan_inputs_id }
        ...
        function savePlan_submit() {
            ...
            restService.savePlan($scope.savePlanData)
                .then(data => {
                    console.log(data);
                    savePlan_close();
            ...
            if (JSON.stringify($route.current.params).length > 2) {
                for (let key in $route.current.params) {
                    try {
                        ...
                        let total = 0;
                        $scope.outputData.plan_details.forEach((row, idx, arr) => {
                        ...
                        for (let key in $scope.outputData.summary_metrics) {
                            $scope.outputData.summary_metrics[key + '_disp'] = numberWithCommas($scope.outputData.summary_metrics[key])
                        }
                        $scope.outputData.plan_details_disp = [];
                        $scope.outputData.plan_details.forEach(row => {
                            let row_disp = [];
                            row.forEach(col => {
                                let col_disp = numberWithCommas(col);
                        ...
                        // $scope.chartDataSeries = $scope.outputData.graphicalDisplayData[$scope.chartDisplayType];
                        $scope.outputData.graphicalDisplayData.FTE.forEach(i => {
                            i.displayed = true;
                        });
                        $scope.outputData.graphicalDisplayData.AI.forEach(i => {
                            i.displayed = true;
                        });
