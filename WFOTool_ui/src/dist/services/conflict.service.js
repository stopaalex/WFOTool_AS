...
var conflictService = angular.module('conflictService', []).service('conflictService', function ($rootScope) {
  var _this = this;

  var conflictService = this;
  conflictService.inputs = {
    checkForConflicts: function checkForConflicts(data, step, cccomparators, rfcomparators) {
      var return_object = {
        conflicts: [],
        exist: false // --- checking the case constraints

      };
      var conflictObject = JSON.parse(JSON.stringify(data));

      for (var key in conflictObject) {
        // string
        if (typeof conflictObject[key] === 'string') {
          conflictObject[key] = '';
        } else {
          for (var key_ in conflictObject[key]) {
            if (conflictObject[key][key_].length) {
              // list
              conflictObject[key][key_].forEach(function (o, i, a) {
                for (var k in o) {
                  o[k] = '';
                }
              });
              ...
              // object
              for (var key__ in conflictObject[key][key_]) {
      ...
      data.cc.constraints.forEach(function (cont, idx, arr) {
        // --- create the equation for checking against the available inventory
        var equation = data.ai[cont.case_type].cases + cccomparators[cont.comparator] + cont.volume; // --- conflict found between this case and the worker/case type
        //
        ...
        arr.forEach(function (cont_, idx_, arr_) {
          if (idx !== idx_) {
              ...
              if (cccomparators[cont.comparator] === '>') {
                if (cccomparators[cont_.comparator] === "=" || cccomparators[cont_.comparator] === "<=" || cccomparators[cont_.comparator] === "<") {
                  if (cont.volume >= cont_.volume) {
                    return_object = _this.inputs.createCCConflictObject(return_object, idx, idx_);
                  }
                } // at least
                ...
                if (cccomparators[cont_.comparator] === '=') {
                  ...
                  if (cont.volume > cont_.volume) {
                    return_object = _this.inputs.createRFConflictObject(return_object, idx, idx_);
                  }
                } // no more than
      ...
      data.rf.flexibility.forEach(function (flex, idx, arr) {
        // --- firist check if there are conflicts between the flexiblity and the workers
        // create the total worker hours
        var totalWorkerHours = data.fte[flex.worker].fte * 2000 * (flex.percent / 100); // create the available hours to for the worker to work

        var availableCaseHours = data.ai[flex.case_type].cases * data.ai[flex.case_type].hours;
        var comparator = flex.comparator.toLowerCase() === 'exactly' ? '===' : rfcomparators[flex.comparator];
        var equation = availableCaseHours + comparator + totalWorkerHours; // --- conflict found between this case and the worker/case type
        //
        ...
        arr.forEach(function (flex_, idx_, arr_) {
          if (idx !== idx_) {
      ...
      if (return_object.conflicts.length > 0) {
        return_object.conflicts.sort(function (a, b) {
          if (a.conflictString > b.conflitString) {
            return 1;
          } else if (a.conflictString < b.conflictString) {
            return -1;
          } else {
            return 0;
          }
    ...
    createRFConflictObject: function createRFConflictObject(obj, i, i_) {
      obj.conflicts.push({
        step: 'Resource Flexibility',
        id: i,
        id_: i_,
    ...
    createCCConflictObject: function createCCConflictObject(obj, i, i_) {
      obj.conflicts.push({
        step: 'Case Constraints',
        id: i,
        id_: i_,
  ...
  conflictService.alerts = {
    createConflictAlert: function createConflictAlert(conflictArr) {
      var conflictEle = document.getElementById('conflictAlerts');
      var html = '';
      var htmlclick = '';
      conflictArr.forEach(function (a) {
        // console.log(a);
        if (!a.worker && !a["case"]) {
          htmlclick = '\
      ...
      _this.alerts.checkForConflictHighlights();
    },
    checkForConflictHighlights: function checkForConflictHighlights() {
      setInterval(function () {
        var highlightedEles = document.querySelectorAll('.highlight');

        if (highlightedEles.length > 0) {
          for (var i = 0; i < highlightedEles.length; i++) {
            _this.alerts.clearHighlight(highlightedEles[i]);
          }
        }
      }, 500);
    },
    clearHighlight: function clearHighlight(ele) {
      setTimeout(function () {
