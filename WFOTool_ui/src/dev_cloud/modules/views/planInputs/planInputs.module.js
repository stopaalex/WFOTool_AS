...
var planInputs = angular.module('planInputs', []).directive('planInputs', function () {
  return {
  ...
  $scope.step = ''; // available steps

  $scope.stepsArr = ['FTE', 'AvailableInventory', 'CaseConstraints', 'ResourceFlexibility']; // base inputData object

  $scope.inputData = {};
  $scope.inputData_rigid = {};
  $scope.validData = true; // the saved plans

  $scope.savedPlans = []; // FTE specific vars
  // modal boolean
  ...
  $scope.ruleToDelete = {}; // AI specific vars
  //modal boolean
  ...
  $scope.runnablePlan = runnablePlan; // validation funcs
  ...
  function workersScroll_scroll(dir, _int) {
    var distance = Number;
    ...
    var ele = document.getElementById('workers');
  ...
  function selectSavedPlan_uploadPlan(plan) {
    ...
    if (JSON.stringify($scope.inputData) === JSON.stringify(plan)) {
      alertService.createAlert('warning', 'Plan Already Selected', 5000); // alert('plan already selected');
    } else {
  ...
  function deleteRule_submit(data) {
    ...
    deleteRule_close(); // $scope.deleteRuleModalShown = false;
    // $scope.ruleToDelete = {};
  }
  ...
  function validateData(data) {
    if (data) {
      var conflicts = conflictService.inputs.checkForConflicts(data, $scope.step, $scope.comparators, $scope.flexiblityComparators);
  ...
  function validateStep(step, dir) {
    if (step === 'FTE') {} else if (step === 'AvailableInventoroy') {} else if (step === 'CaseConstraints') {} else if (step === 'ResourceFlexibility') {}
  }
  ...
  function goToPrevStep(step) {
    var newStepIdx = 0;
    $scope.stepsArr.forEach(function (stp, idx, arr) {
      if (step === stp) {
        newStepIdx = idx - 1;
        messageService.broadcastMessage('changeStep', {
          step: $scope.stepsArr[newStepIdx]
        });
      }
    });
  }
  ...
  function runPlan(d) {
    ...
    data['modified'] = JSON.stringify(today); // ---setting local storage to the run plan so it can be returned to

    localStorage.setItem('_WFOTool_workingPlan', JSON.stringify(data));
    restService.runPlan(data).then(function (data) {
      // console.log(data);
      if (data.statusCode === 200) {
        ...
        $scope.$apply();
        alertService.createAlert('success', 'Your plan was successfully run!', 2500); // localStorage.removeItem('_WFOTool_workingPlan');

        loaderService.removeLoader();
      } else if (data.statusCode === 418) {
        $location.path('/output').search(encodeURI(JSON.stringify(data.body)));
        $scope.$apply();
        alertService.createAlert('warning', 'Your plan was run but returned a 418 error with some data showing errors (NaN)!', 10000); // localStorage.removeItem('_WFOTool_workingPlan');
  ...
  function getPlans_default() {
    ...
    if (localStorage.getItem('_WFOTool_workingPlan')) {
      var d = JSON.parse(localStorage.getItem('_WFOTool_workingPlan'));
      $scope.inputData['fte'] = {};
      $scope.inputData['ai'] = {};
      Object.keys(d.fte).sort().forEach(function (key) {
        $scope.inputData.fte[key] = {};
        Object.keys(d.fte[key]).forEach(function (key_) {
          $scope.inputData.fte[key][key_] = d.fte[key][key_];
        });
      });
      Object.keys(d.ai).sort().forEach(function (key) {
        $scope.inputData.ai[key] = {};
        Object.keys(d.ai[key]).forEach(function (key_) {
      ...
      $scope.inputData_rigid = JSON.parse(JSON.stringify($scope.inputData)); // $scope.inputData_rigid = JSON.parse(JSON.stringify(localStorage.getItem('_WFOTool_workingPlan')));
    ...
    restService.getDefaultPlan().then(function (data) {
      var d = JSON.parse(JSON.stringify(data.body));
      ...
      $scope.inputData_rigid = JSON.parse(JSON.stringify($scope.inputData)); // $scope.inputData = data.body;

      $scope.$apply();
      $timeout(function () {
        generateListeners_scroll();
      }, 250); // console.log('saved data loaded for default');
  ...
  function getPlans_savedPlans() {
    restService.getSavedPlans().then(function (data) {
      var saved_array = data.body.Items;
      saved_array.forEach(function (plan, idx, arr) {
        $scope.savedPlans[idx] = {};
        $scope.savedPlans[idx]['fte'] = {};
        $scope.savedPlans[idx]['ai'] = {};
        Object.keys(plan.fte).sort().forEach(function (key) {
          $scope.savedPlans[idx].fte[key] = {};
          Object.keys(plan.fte[key]).forEach(function (key_) {
        ...
        Object.keys(plan.ai).sort().forEach(function (key) {
          ...
          Object.keys(plan.ai[key]).forEach(function (key_) {
      ...
      $scope.$apply();
      restService.getDefaultPlan().then(function (d_) {
        var d = JSON.parse(JSON.stringify(d_.body));
        var _d = {};
        _d['fte'] = {};
        _d['ai'] = {};
        Object.keys(d.fte).sort().forEach(function (key) {
  ...
  function init() {
    ...
    generateListeners(); // getPlans_default('data/defaultInputs.json');
